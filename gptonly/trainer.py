...
class Trainer:
    ...
    def metric_output(self, metrics):
        return (f"loss={metrics['avg_loss']: .4f}, f1={metrics['eot_f1']: .4f}, acc={metrics['eot_acc']: .4f}, "
                f"recall={metrics['eot_recall']: .4f}, rouge2={metrics['rouge_mean']: .4f}")
    ...
    def train_epoch(self, train_dl):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            predicted_token_ids = torch.argmax(out.logits, dim=-1)
            pred_label.append((predicted_token_ids, output_ids))
