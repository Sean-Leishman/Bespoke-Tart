...
from mbespoketart import MaskedBert
...
def main(config):
    ...
    if config.bert_type == 'bert':
        ...
        model = MaskedBert(pretrained_model_name=config.bert_pretraining, bert_finetuning=bert_finetuning, config=config)
    else:
        logging.getLogger(__name__).info(
    ...
    if config.evaluate == 'false':
        ...
        history = trainer.train(train_dl, test_dl, scheduler)
    else:
        test_dl = DataLoader(TranscriptDataset(
